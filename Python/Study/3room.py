...
class Room(object):
    ...
    def __init__(self):
        self.door_list = Room.door_list # 여기서는 모든방이지만 각각의 방에서 Override할 예정.
    ...
    def move(self):
        ...
        if self.door in self.door_list:
            self.next_door = Map.room_dict[self.door] # 텍스트로 받은 방을 Map의 사전으로 방 객체를 호출 (이미 인스턴스화)
        self.next_door.enter() # 방 객체 (A,B,C)는 Room을 상속받았으므로 enter가 묵시적으로 상속.
...
class Engine(object):
    ...
    def init_point(self): # 시작위치를 랜덤으로 지정할것임.
        ...
        self.shuffle_list = self.shuffle(self.door_list) # Engine 내에 함수가 있기에 self.shuffle을 사용해야함. (self는 기본 인스턴스)
